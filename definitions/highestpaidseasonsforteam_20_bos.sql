WITH all_lines AS (
 ...
 SELECT player_id,year_id,STRING_AGG(team_id,',' ORDER BY first_appearance) teams,BOOL_OR(team_id='BOS') is_team FROM (SELECT player_id,year_id,team_id,MIN(source_order) first_appearance FROM all_lines GROUP BY player_id,year_id,team_id) GROUP BY player_id,year_id
)
