...
FROM careers c JOIN teams t USING(player_id) CROSS JOIN floor
WHERE c.salary>0 AND c.max_year>=(SELECT MAX(year_id) FROM read_parquet('{{BAT}}')) ORDER BY c.salary/(c.career_war-floor.lowest_war) DESC,c.player_order LIMIT 20
