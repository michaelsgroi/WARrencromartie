WITH all_lines AS (
 ...
 SELECT player_id,FIRST(name_common ORDER BY source_order) AS name,year_id,SUM(war) season_war,CAST(ROUND(AVG(salary)) AS BIGINT) salary,MIN(source_order) season_order
...
SELECT s.name,ROUND(s.season_war,2) war,s.salary,s.year_id AS "year",t.teams FROM seasons s JOIN teams t USING(player_id,year_id)
