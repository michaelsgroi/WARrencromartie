...
team_lines AS (
    SELECT l.player_id, l.team_id,
        ROW_NUMBER() OVER (
            PARTITION BY l.player_id
            ORDER BY s.season_first_appearance, l.source_order
        ) AS career_order
    FROM all_lines l
    JOIN player_seasons s USING (player_id, year_id)
),
player_teams AS (
    SELECT player_id, team_id, MIN(career_order) AS first_appearance
    FROM team_lines
...
teams AS (
    SELECT player_id,
        ...
        BOOL_OR(team_id = 'BOS') AS played_for_team
    FROM player_teams
    GROUP BY player_id
)
SELECT c.name,
...
ORDER BY c.career_war, c.first_appearance
