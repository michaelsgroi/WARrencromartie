WITH all_lines AS (
 ...
 SELECT player_id,year_id,SUM(war) season_war,CAST(ROUND(AVG(salary)) AS BIGINT) season_salary FROM all_lines GROUP BY player_id,year_id
), player_info AS (
...
FROM careers c JOIN teams t USING(player_id) WHERE c.career_war<0 ORDER BY c.salary DESC,c.player_order LIMIT 20
