...
class BrReports(
    ...
    private fun rosterRetention(
        team: String,
        yearRange: IntRange,
        yearsRetained: Int,
        topN: Int,
    ): Report<Pair<Pair<Int, Int>, Int>> {
    ...
    private fun averageRosterRetention(
        yearRange: IntRange,
        yearsRetained: Int,
        topN: Int,
    ): Report<Pair<Double, Pair<String, Int>>> {
        val rosters = brWarDaily.rosters
        val allTeams = rosters.groupBy { it.rosterId.team }
        val allTeamsMinMaxSeasons =
            allTeams
                .map { entry ->
        ...
        val teamsToCompute =
            allTeamsMinMaxSeasons
                .filter { entry ->
                    entry.value.first!! <= yearRange.first && entry.value.second!! >= yearRange.last
                }.keys
                .sorted()
        val teamRetention: List<Pair<Double, Pair<String, Int>>> =
            teamsToCompute
                .map { team ->
...
//        println(retentionsStr)
    ...
    private fun bestCareerWar(
        team: String,
        topN: Int,
    ): Report<Career> =
        buildReport(listOf(team, topN), getCareerFormatter(includeWar = true)) {
            brWarDaily.careers
                .filter { it.teams().contains(team.uppercase()) }
                .sortedByDescending { career -> career.war() }
        }

    private fun worstCareerWar(
        team: String,
        topN: Int,
    ): Report<Career> =
        buildReport(listOf(team, topN), getCareerFormatter(includeWar = true)) {
    ...
    private fun bestCareerWar(topN: Int): Report<Career> =
        buildReport(topN, getCareerFormatter(includeWar = true)) {
            brWarDaily.careers
...
//        println(warIndexesOfInitialYearPlayersWhoAreOnTargetYearRoster.toSortedMap().map { "${it.key}:${it.value.playerName}" }.joinToString("\n"))
    ...
    private fun averageSeasonWar(topN: Int): Report<Career> =
        buildReport(topN, getCareerFormatter(includeWar = true, includeAverageWar = true)) {
            brWarDaily.careers
                .sortedByDescending { career ->
    ...
    private fun consecutiveSeasonsWithWarOver(
        topN: Int,
        minWar: Double,
    ): Report<Career> =
        buildReport(listOf(topN, minWar), getCareerFormatter(includeWar = false)) {
            brWarDaily.careers
                .sortedByDescending { career -> career.consecutiveSeasonsOver(minWar).size }
                .take(topN)
                .map { career ->
    ...
    private fun bestTopNSeasons(
        ...
        buildReport(listOf(topNSeasons, topNPlayers, desc), getCareerFormatter()) {
            brWarDaily.careers
                ...
                .map { career ->
                    career.copy(seasonsPredicate = {
                        it.season in
                            career
                                .seasons()
                                .sortedByDescending { season -> season.war }
                                .take(topNSeasons)
    ...
    private fun playersWhoseNameStartsWith(startsWith: String): Report<Career> =
        buildReport(startsWith, getCareerFormatter()) {
            brWarDaily.careers
                .filter { it.playerName.lowercase().startsWith(startsWith.lowercase()) }
    ...
    private fun playersWhoseNameContains(contains: String) =
        buildReport(contains, getCareerFormatter()) {
            brWarDaily.careers
                .filter { contains.lowercase() in it.playerName.lowercase() }
                .sortedByDescending { it.war() }
        }

    private fun highestPeakSeasonWarWithCareerWarUnder(
        topN: Int,
        maxWar: Int,
        ...
        brWarDaily.seasons
            .asSequence()
            .filter { it.playerId in careersUnderMaxWar.keys }
            .sortedByDescending { it.war }
            .take(topN)
            .map { careersUnderMaxWar[it.playerId]!! }
            .toList()
    }

    private fun lowestPeakSeasonWarWithCareerWarOver(
        topN: Int,
        minWar: Int,
        ...
        val minimumMaxSeasons =
            careersOverMinWar.values
                .map { career -> career.seasons().maxBy { season -> season.war } }
    ...
    private fun highestPaidSeasonsForTeam(
        topN: Int,
        team: String,
    ) = buildReport(listOf(topN, team), getSeasonFormatter(includeSalary = true)) {
        brWarDaily.seasons
            .filter { team.lowercase() in it.teams.map { team -> team.lowercase() } }
            .sortedByDescending { it.salary }
            .take(topN)
            .toList()
    }

    private fun lowestPaidWarSeasonsByWarSince(
        topN: Int,
        desc: String,
        yearFilter: (Int) -> Boolean = { true },
    ) = buildReport(
        ...
        brWarDaily.seasons
            .filter { it.salary > 0 && yearFilter(it.season) }
            .sortedByDescending { it.war / it.salary }
            .take(topN)
    }
    ...
    private fun lowestPaidWarCareersReportFunction(
        topN: Int,
        yearFilter: (Int) -> Boolean,
    ): () -> List<Career> =
        {
            brWarDaily.careers
                .filter { it.salary() > 0 && yearFilter(it.seasons().maxOf { sl -> sl.season }) }
                .sortedByDescending { it.war() / it.salary() }
                .take(topN)
        }
    ...
    private fun highestPaidCareersByWarOfPlayersWhoAreCurrentlyActive(topN: Int) =
        buildReport(topN, getCareerFormatter(includeSalary = true)) {
            ...
            careers
                .filter { it.salary() > 0 && it.seasons().maxOf { sl -> sl.season } >= 2022 }
                .sortedByDescending { it.salary() / (it.war() - lowestWar) }
                .take(topN)
        }
    ...
    private fun theSteveBalboniAllStars() =
        buildReport(getCareerFormatter()) {
            brWarDaily.careers
                .sortedWith(compareBy({ it.war() }, { it.playerName }))
    ...
    private fun bestOrWorstNOfTeam(
        team: String,
        topN: Int,
        best: Boolean,
    ): Report<Career> =
    ...
    private fun bestRostersByFranchise(verbosity: Verbosity = VERBOSE) =
        buildReport(verbosity, getRosterFormatter(verbosity)) {
            ...
            teams
                .map { team -> topRosters.first { it.rosterId.team == team } }
    ...
    private fun season(
        ...
        brWarDaily.rosters
            .first { roster -> roster.rosterId == rosterId }
            .players
    ...
    private fun <T> buildReport(
        ...
        run: () -> List<T>,
    ): Report<T> = buildReport(emptyList(), formatter, run)

    private fun <T> buildReport(
        argsString: Any,
        formatter: BrReportFormatter<T>,
        run: () -> List<T>,
    ): Report<T> = buildReport(listOf(argsString), formatter, run)
    ...
    companion object {
        ...
        private fun String.toFileName(): String = replace(pattern, "$0").lowercase()
    }
}
